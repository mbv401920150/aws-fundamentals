    ...
    public async Task FunctionHandler(S3Event evnt, ILambdaContext context)
    {
        ...
        foreach (var record in eventRecords)
        {
            ...
            try
            {
                ...
                // Flag added to avoid an infinite loop
                if (response.Metadata["x-amz-meta-resized"] == true.ToString())
                {
                    context.Logger.LogInformation($"Item with the key {s3Event.Object.Key} has been resized.");
                    continue;
                }

                await using var itemStream = await S3Client.GetObjectStreamAsync(
                    s3Event.Bucket.Name,
                    s3Event.Object.Key,
